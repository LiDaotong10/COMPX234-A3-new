...
def handle_client(client_socket):
    ...
    try:
        while True:
            ...
            len_data = receive_n(client_socket, 3)
            if not len_data:
                break
            try:msg_len = int(len_data.decode('utf-8'))
            except ValueError:
                increment_stat("error_count")
                response = "ERR Invalid message length"
                
                resp_len = len(response) + 4  # 3位长度 + 空格 + 响应体
                client_socket.sendall(f"{resp_len:03d} {response}".encode('utf-8'))
                break
            
            msg_body = receive_n(client_socket, msg_len - 3)
            if not msg_body:
                break
            message_buffer = msg_body.decode('utf-8').strip()
            ...
            resp_body = response.strip()
            resp_total_len = 3 + 1 + len(resp_body)  # 3位长度 + 空格 + 响应体
            resp_message = f"{resp_total_len:03d} {resp_body}"
            client_socket.sendall(resp_message.encode('utf-8'))
